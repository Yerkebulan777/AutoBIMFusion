...
internal static class ViewportLayoutExporter
{
    ...
    public static async Task<string> ExportToTempAsync(string sourceFilePath, string fileName, OperationLogger log)
    {
        ...
        using (Database db = new(false, true))
        {
            ...
            {
                log.Warn($"{fileName}: Р»РёСЃС‚С‹ РЅРµ РЅР°Р№РґРµРЅС‹");
                return string.Empty;
            }

            // Р’СЃСЏ Р»РѕРіРёРєР° СЂР°Р±РѕС‚С‹ СЃ Р‘Р” С‚РµРїРµСЂСЊ РІ Side-Database.
            List<LayoutViewportInfo> vps = ViewportCollector.Collect(db, layoutName);
            log.Info($"VP: РЅР°Р№РґРµРЅРѕ {vps.Count}");
            ...
            if (frameBounds.HasValue)
            {
                ...
                log.Info($"VP: РѕС‡РёС‰РµРЅРѕ {erased} РѕР±СЉРµРєС‚РѕРІ");
            }
            ...
            // РџСЂРѕРІРµСЂСЏРµРј РЅР° РЅР°Р»РёС‡РёРµ СЂР°СЃС‚СЂРѕРІ РґР»СЏ РІРЅРµРґСЂРµРЅРёСЏ.
            // РСЃРїРѕР»СЊР·СѓРµРј Handles, С‚Р°Рє РєР°Рє РїСЂРё РѕС‚РєСЂС‹С‚РёРё РґРѕРєСѓРјРµРЅС‚Р° ObjectIds РёР·РјРµРЅСЏС‚СЃСЏ.
        ...
        log.Info($"VP: СЌРєСЃРїРѕСЂС‚ Р·Р°РІРµСЂС€РµРЅ ({fileName})");
        return tempPath;
    }

    private static bool CheckIfNeedsOle(Database db, HashSet<long> paperClonedHandles, string sourceDir, OperationLogger log)
    {
        // РќР°Рј РЅРµ РЅСѓР¶РЅС‹ РїРѕР»РЅС‹Рµ РґР°РЅРЅС‹Рµ, С‚РѕР»СЊРєРѕ С„Р°РєС‚ РЅР°Р»РёС‡РёСЏ С…РѕС‚СЏ Р±С‹ РѕРґРЅРѕРіРѕ РїРѕРґС…РѕРґСЏС‰РµРіРѕ РёР·РѕР±СЂР°Р¶РµРЅРёСЏ
    ...
    private static async Task RunOleEmbeddingAsync(string tempPath, HashSet<long> paperClonedHandles, string sourceDir, OperationLogger log)
    {
        ...
        if (tempDoc is null)
        {
            log.Warn($"РњРµ СѓРґР°Р»РѕСЃСЊ РѕС‚РєСЂС‹С‚СЊ РІСЂРµРјРµРЅРЅС‹Р№ С„Р°Р№Р» РґР»СЏ OLE-РІСЃС‚СЂР°РёРІР°РЅРёСЏ: {tempPath}");
            return;
        }
        ...
        {
            log.Error(ex, "РћС€РёР±РєР° РїСЂРё OLE-РІСЃС‚СЂР°РёРІР°РЅРёРё РІРѕ РІСЂРµРјРµРЅРЅС‹Р№ С„Р°Р№Р»");
        }
        finally
        {
    ...
    {
        log.Info($"VP: РјСѓР»СЊС‚Рё-СЂРµР¶РёРј ({vps.Count} С€С‚)");
        ...
        log.Info(
            $"VP main#{mainOriginal.Number}: РёСЃС…РѕРґРЅС‹Р№ scale={mainOriginal.CustomScale:F6}, " +
            $"СЂР°Р±РѕС‡РёР№ scale={mainClamped.CustomScale:F6}, clampRatio={clampRatio:F6}, " +
            $"С†РµРЅС‚СЂ={GeometryUtils.FormatPoint(mainOriginal.ViewCenter)}");
        ...
        foreach (LayoutViewportInfo aux in vps)
        {
            ...
            if (toClone.Count > 0)
            {
                ...
                // РЈРґР°Р»СЏРµРј РѕСЂРёРіРёРЅР°Р»С‹ aux VP, РєРѕС‚РѕСЂС‹С… РЅРµС‚ РІ РіР»Р°РІРЅРѕРј VP.
                _ = ViewportTransformer.EraseEntitiesOutsideMainWindow(db, toClone, modelEntities, mainOriginal.ModelWindow, log);
                log.Info($"VP #{aux.Number}: РѕР±СЂР°Р±РѕС‚Р°РЅРѕ {cloned.Count} РѕР±СЉРµРєС‚РѕРІ");
            }
            else
            {
                log.Info($"VP #{aux.Number}: 0 РѕР±СЉРµРєС‚РѕРІ");
            }
        }
    ...
    /// <summary>
    /// РћРґРёРЅ VP: Р·Р°Р¶РёРјР°РµС‚ РјР°СЃС€С‚Р°Р±, РјР°СЃС€С‚Р°Р±РёСЂСѓРµС‚ Model Space РµСЃР»Рё РЅСѓР¶РЅРѕ, РїРµСЂРµРЅРѕСЃРёС‚ Paper РІ Model Space.
    /// </summary>
    ...
    private static LayoutViewportInfo ClampMainVpScale(LayoutViewportInfo vp, OperationLogger log)
    {
        ...
        {
            // РњР°СЃС€С‚Р°Р± РєСЂСѓРїРЅРµРµ 1:100 вЂ” Р·Р°Р¶РёРјР°РµРј, С‡С‚РѕР±С‹ РЅРµ РїРѕР»СѓС‡РёС‚СЊ СЃР»РёС€РєРѕРј Р±РѕР»СЊС€РѕР№ Model Space РїРѕСЃР»Рµ СЌРєСЃРїРѕСЂС‚Р°.
            log.Info($"VP #{vp.Number}: РјР°СЃС€С‚Р°Р± 1:{multiplier:F0} Р·Р°Р¶Р°С‚ РґРѕ 1:{MaxScaleMultiplier:F0}");
            return vp with { CustomScale = 1.0 / MaxScaleMultiplier };
        }

        log.Info(
            $"VP #{vp.Number}: РјР°СЃС€С‚Р°Р± 1:{multiplier:F0} (Р±РµР· Р·Р°Р¶РёРјР°), " +
            $"customScale={vp.CustomScale:F6}");
        return vp;
    }

    /// <summary>
    /// РќРµС‚ VP: РјР°СЃС€С‚Р°Р±РёСЂСѓРµС‚ Рё РїРµСЂРµРЅРѕСЃРёС‚ Paper-СЃРѕРґРµСЂР¶РёРјРѕРµ РІ Model Space СЃ РјР°СЃС€С‚Р°Р±РѕРј 1:MaxScaleMultiplier.
    /// </summary>
    ...
    {
        log.Info($"VP: РЅРµС‚ РІРёРґРѕРІС‹С… СЌРєСЂР°РЅРѕРІ, РјР°СЃС€С‚Р°Р± 1:{MaxScaleMultiplier:F0}");
    ...
    private static List<(ObjectId id, string path, Extents3d bounds)> CollectRasterImages(Database db, HashSet<long> paperClonedHandles, string sourceDir, OperationLogger log)
    {
        ...
        foreach (ObjectId id in ms)
        {
            ...
            {
                fileNotFoundCount++;
                log.Warn($"RasterImage Handle={id.Handle}: С„Р°Р№Р» РЅРµ РЅР°Р№РґРµРЅ: {def.SourceFileName}");
                continue;
            }
            ...
            {
                tooLargeCount++;
                log.Info($"RasterImage Handle={id.Handle}: С„Р°Р№Р» {Path.GetFileName(resolvedPath)} СЃР»РёС€РєРѕРј Р±РѕР»СЊС€РѕР№ ({fileSize / (1024.0 * 1024.0):F1} РњР‘ > 5 РњР‘), РїСЂРѕРїСѓСЃРєР°РµРј OLE-РєРѕРЅРІРµСЂС‚Р°С†РёСЋ");
                continue;
            }
    ...
    private static async Task EmbedSingleRasterAsync(Document doc, Database db, ObjectId rasterId, string path, Extents3d targetBounds, OperationLogger log)
    {
        try
        {
            ...
            log.Info($"OLE РІС?С‚Р°РІРєР°: РґРѕ РІС?С‚Р°РІРєРё РѕР±СЉРµРєС‚РѕРІ РІ MS: {snapshotBefore.Count}, С‚РѕС‡РєР° {targetBounds.MinPoint}, С„Р°Р№Р» {Path.GetFileName(path)}");

            if (!TryCopyImageToClipboard(path, log))
            {
                log.Warn($"РњРµ СѓРґР°Р»РѕСЃСЊ РїРѕРјРµСЃС‚РёС‚СЊ РёР·РѕР±СЂР°Р¶РµРЅРёРµ РІ Clipboard: {path}");
                return;
            }
            ...
            if (oleId.IsNull)
            {
                log.Warn($"PASTECLIP РЅРµ СЃРѕР·РґР°Р» РЅРѕРІС‹Р№ OLE2FRAME РґР»СЏ {path}. РџСЂРѕРІРµСЂСЊС‚Рµ OLEQUALITY Рё Clipboard.");
                return;
            }

            log.Info($"РќР°Р№РґРµРЅ РЅРѕРІС‹Р№ OLE2FRAME: Handle={oleId.Handle}, Id={oleId}");

            using Transaction tr = db.TransactionManager.StartTransaction();
            if (tr.GetObject(oleId, OpenMode.ForWrite) is not Ole2Frame ole)
            {
                log.Warn($"РњР°Р№РґРµРЅРЅС‹Р№ РѕР±СЉРµРєС‚ РЅРµ СЏРІР»СЏРµС‚СЃСЏ Ole2Frame: С‚РёРї={oleId.ObjectClass.DxfName}");
                tr.Commit();
                return;
            }
            ...
            {
                originalImage.Erase();
                log.Info($"РЈРґР°Р»С‘РЅ РёСЃС…РѕРґРЅС‹Р№ RasterImage: {rasterId.Handle}");
            }
        ...
        {
            log.Warn(ex, $"РћС€РёР±РєР° РїСЂРё РІС?С‚СЂР°РёРІР°РЅРёРё OLE: {path}");
        }
    }
    ...
    private static bool TryCopyImageToClipboard(string path, OperationLogger log)
    {
        ...
        {
            log.Warn(ex, $"РњРµ СѓРґР°Р»РѕСЃСЊ СЃРєРѕРїРёСЂРѕРІР°С‚СЊ РёР·РѕР±СЂР°Р¶РµРЅРёРµ РІ Clipboard: {path}");
            return false;
        }
    }
    ...
    private static ObjectId FindNewOle2Frame(Database db, HashSet<ObjectId> snapshotBefore, OperationLogger log)
    {
        ...
        foreach (ObjectId id in ms)
        {
            ...
            {
                // Р•СЃР»Рё СЃРѕР·РґР°РЅРѕ РЅРµСЃРєРѕР»СЊРєРѕ РѕР±СЉРµРєС‚РѕРІ, Р±РµСЂРµРј РїРѕСЃР»РµРґРЅРёР№ РїРѕ Handle РєР°Рє РЅР°РёР±РѕР»РµРµ РІРµСЂРѕСЏС‚РЅС‹Р№
    ...
    private static bool ResizeOleToTarget(Ole2Frame ole, Extents3d targetBounds, OperationLogger log)
    {
        ...
        {
            log.Warn($"Р¦РµР»РµРІРѕР№ СЂР°Р·РјРµСЂ OLE РЅРµРєРѕСЂСЂРµРєС‚РµРЅ: {targetWidth:F4} x {targetHeight:F4}");
            return false;
        }
        ...
        {
            log.Warn("OLE Bounds РЅРµ РѕРїСЂРµРґРµР»РµРЅС‹ РґРѕ РјР°СЃС€С‚Р°Р±РёСЂРѕРІР°РЅРёСЏ. РџСЂРѕР±СѓРµРј Position3d fallback.");
        ...
        log.Info($"OLE СЂР°Р·РјРµСЂ РґРѕ РјР°СЃС€С‚Р°Р±РёСЂРѕРІР°РЅРёСЏ: {initialWidth:F4} x {initialHeight:F4}, С†РµР»РµРІРѕР№: {targetWidth:F4} x {targetHeight:F4}");
        ...
        {
            log.Warn(
                $"OLE Bounds РЅРµРєРѕСЂСЂРµРєС‚РЅС‹: {initialWidth:F4}x{initialHeight:F4}. " +
                "РџСЂРѕРїСѓСЃРєР°РµРј WcsWidth/Height, РёСЃРїРѕР»СЊР·СѓРµРј Position3d.");
            return TryApplyPositionFallback(ole, targetBounds, log);
        }
        ...
        {
            log.Warn("РџРѕСЃР»Рµ WcsWidth/WcsHeight РЅРµ СѓРґР°Р»РѕСЃСЊ РїРѕР»СѓС‡РёС‚СЊ Bounds. РџСЂРѕР±СѓРµРј Position3d fallback.");
            return TryApplyPositionFallback(ole, targetBounds, log);
        }
        ...
        log.Info($"OLE СЂР°Р·РјРµСЂ РїРѕСЃР»Рµ WcsWidth/Height: {resizedWidth:F4} x {resizedHeight:F4}");
        ...
        log.Warn(
            $"WcsWidth/WcsHeight РЅРµ РїСЂРёРјРµРЅРёР»РёСЃСЊ РєРѕСЂСЂРµРєС‚РЅРѕ: С‚РµРєСѓС‰РёР№={resizedWidth:F4}x{resizedHeight:F4}, " +
            $"С†РµР»РµРІРѕР№={targetWidth:F4}x{targetHeight:F4}. РџСЂРѕР±СѓРµРј Position3d fallback.");
        return TryApplyPositionFallback(ole, targetBounds, log);
    }
    ...
    private static bool TryApplyPositionFallback(Ole2Frame ole, Extents3d targetBounds, OperationLogger log)
    {
        try
        {
            ...
            if (boundsAfterFallback.HasValue)
            {
                ...
                log.Info($"OLE СЂР°Р·РјРµСЂ РїРѕСЃР»Рµ Position3d fallback: {width:F4} x {height:F4}");
            }
        ...
        {
            log.Warn(ex, "Position3d fallback РЅРµ СЃСЂР°Р±РѕС‚Р°Р».");
            return false;
        }
    }
    ...
    private static void AlignOleToTargetMinPoint(Ole2Frame ole, Extents3d targetBounds, OperationLogger log)
    {
        ...
        {
            log.Warn("РњРµ СѓРґР°Р»РѕСЃСЊ РІС‹СЂРѕРІРЅСЏС‚СЊ OLE: Bounds РѕС‚СЃСѓС‚СЃС‚РІСѓСЋС‚.");
            return;
        }
        ...
        {
            log.Warn("TransformBy РІС‹РїРѕР»РЅРµРЅ, РЅРѕ Bounds OLE РЅРµРґРѕСЃС‚СѓРїРЅС‹ РїРѕСЃР»Рµ СЃРґРІРёРіР°.");
            return;
        }
        ...
        {
            log.Info($"TransformBy СЃСЂР°Р±РѕС‚Р°Р», СЃРјРµС‰РµРЅРёРµ {movedDistance:F4}");
            return;
        }

        log.Warn("TransformBy(Displacement) РЅРµ РёР·РјРµРЅРёР» OLE. РџСЂРѕР±СѓРµРј Position3d fallback.");
