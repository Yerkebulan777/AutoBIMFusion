...
Помимо `MERGEDWG` и `MERGEDWG_DIAG_TEST`, плагин сохраняет отдельные пользовательские команды: `SMART_MERGE_TEXT`, `MergeTextStyles`, `JOIN_LINES`, `CreateETransmitZip`, `ExportTextStylesToMd` и `ExportDimStylesToMd`. Они доступны как AutoCAD `CommandMethod` entrypoints и через Ribbon, кроме диагностического `MERGEDWG_DIAG_TEST`, который предназначен для повторяемой отладки и скриптового запуска.
