...
- диагностические записи размеров идут с префиксом `[DIM-DIAG]`, снимки таблицы размерных стилей - с `[DIM-STYLE]`, а итоговая группировка по сценариям - с `[DIM-DIAG-SUMMARY]`.
...
- `ScaleModelSpaceObjects`: Применяет масштабирование с ограничением (clamp). Специальная обработка размеров вынесена в `DimensionTransformUtils`, где логируются entity-параметры размера, связанный `DimStyle`, визуальная высота текста/стрелок и extents. `Dimscale` не умножается на `scaleFactor`, потому что `TransformBy()` уже масштабирует геометрию размерного блока; `Dimlfac` остается отдельной компенсацией числового значения.
