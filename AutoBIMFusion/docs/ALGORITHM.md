...
- для обычного и диагностического запуска пишутся только два снимка пользовательских стилей: `before-merge` и `after-merge`. Размерные стили идут с `[DIM-STYLE]`, текстовые стили - с `[TEXT-STYLE]`, ошибки очистки переопределений размеров - с `[DIM-OVERRIDES]`.
