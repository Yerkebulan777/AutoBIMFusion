...
1. В модели собираются все `RasterImage` с валидными `Bounds` и доступным файлом. Изображения, перенесённые с листов (Paper Space) при экспорте Layout, **игнорируются** и остаются как внешние ссылки (связи). Встраиваются только те изображения, которые изначально находились в пространстве Модели.
