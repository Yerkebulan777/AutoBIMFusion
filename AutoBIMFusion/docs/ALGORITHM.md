...
- для обычного и диагностического запуска пишутся только два снимка пользовательских стилей: `before-merge` и `after-merge`. Размерные стили идут с `[DIM-STYLE]`, текстовые стили - с `[TEXT-STYLE]`, очистка переопределений размеров - с `[DIM-OVERRIDES]`.
