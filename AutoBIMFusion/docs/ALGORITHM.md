...
3. Инициализируется `AILog`: сообщения выводятся в редактор AutoCAD, а файловая запись идет через общий `LoggerFactory`/Serilog.
...
6. К каждому клонированному объекту применяется вычисленное смещение (displacement) по оси X для расположения чертежей в один ряд. В этом же проходе собираются ID только новых клонированных размерных объектов (`Dimension`).
7. `DimensionHealer` нормализует новые размерные стили и обрабатывает только собранные новые размеры: удаляет `DSTYLE` overrides из секции `ACAD` xdata, сохраняя остальные xdata, и сбрасывает поворот текста размеров.
8. Вычисляются итоговые мировые границы (world bounds) на основе габаритов клонированных объектов; если габариты недоступны, трансформируются исходные границы.
9. Обновляется переменная `_rightMax`, чтобы задать отступ (gap) для размещения следующего листа.
...
2. **Снимок стилей:** `DimensionStyleDiagnosticUtils.LogStyleSnapshot` пишет финальный `after-merge` снимок пользовательских размерных и текстовых стилей. Очистка размерных overrides уже выполнена в per-file пути `DimensionHealer`, поэтому финализация не делает повторный полный обход размеров.
3. **Очистка (Purge):** `DwgOptimizer.Optimize` удаляет неиспользуемые символы (слои, блоки, стили) с ограничением по количеству проходов (максимум `5` проходов — константа `MaxPurgePasses`).
4. **Сохранение:** Вызывается `SaveMerged`, который сохраняет целевой файл в формате `DwgVersion.AC1032`.
5. **Обновление экрана:** AutoCAD выполняет команды `REGENALL` (регенерация чертежа) и `ZOOM EXTENTS` (показать всё).
...
## 10. План будущей оптимизации (Roadmap)

Для повышения производительности и улучшения структуры кода рекомендуется выполнить следующий рефакторинг:

### Шаг 1: Переход на in-memory конвейер
*   **Изменение `ViewportLayoutExporter`:** Метод должен возвращать объект `Database`, а не путь к временному файлу. Исключить сохранение на диск (`db.SaveAs`).
*   **Обновление `MergeCoordinator`:** Передавать полученную `Database` напрямую в `ReadBounds` и `BlockInserter`. Это уберет 2 лишних цикла чтения/записи файла.
*   **Очистка:** Обеспечить вызов `Dispose()` для `Database` в блоке `finally` координатора.

### Шаг 2: Консолидация трансформаций
*   **Один проход:** В `BlockInserter` объединить циклы клонирования, применения `TransformBy` и очистки специфических свойств объектов (например, размеров).
*   **Транзакции:** Выполнять "лечение" размеров (`DimensionHealer`) в рамках той же транзакции, в которой происходит вставка, чтобы избежать повторного открытия объектов.

### Шаг 3: Унификация логики
*   **Службы размеров:** Создать единый класс `DimensionUtils` для удаления переопределений (overrides), объединив код из `EntityTransformUtils` и `DimensionHealer`.
*   **Проекция листов:** Свести обработку одного и нескольких видовых экранов в `LayoutProjectionProcessor` к единому алгоритму.
