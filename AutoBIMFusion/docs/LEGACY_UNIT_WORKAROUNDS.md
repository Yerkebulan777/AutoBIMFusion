...
Архив старых обходов проблемы скрытого масштабирования AutoCAD при `WblockCloneObjects` между базами с разными `Insunits`/`Measurement`. Все фрагменты заменены единым `DatabaseUnitSyncScope` (`AutoBIMFusion/Application/AcadSupport/DatabaseUnitSyncScope.cs`), дата 2026-05-07.
