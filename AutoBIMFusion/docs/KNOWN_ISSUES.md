...
Алгоритм нормализует размеры во время обработки каждого Viewport до клонирования aux-геометрии. Для каждого видимого Model Space размера создаётся или переиспользуется стиль вида `"{Стиль}_{Множитель}"`, очищаются DSTYLE overrides, `Dimscale` запекается в визуальные параметры стиля и приводится к `1.0`; стиль отвечает за визуальный размер. Числовая поправка измерения хранится на экземпляре размера: при clamp Model Space получает `Dimlfac = 1 / clampRatio`, без clamp ожидается `Dimlfac = 1.0`.
