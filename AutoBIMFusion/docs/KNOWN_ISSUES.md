...
Алгоритм нормализует размеры в подготовленной исходной базе до `WblockCloneObjects`. Для каждого размера создаётся или переиспользуется стиль вида `"{Стиль} - Scale {Множитель}"`, очищаются DSTYLE overrides, `Dimscale` и `Dimlfac` приводятся к `1.0`.

**Риск:** После масштабирования Model Space, переноса Paper Space или трансформации aux viewport высота текста, размер стрелок или числовое значение размера может не совпасть с оригиналом.

**Статус:** Диагностика (`[DIM-NORMALIZE]`, `[DIM-STYLE]`, snapshots) пишется в лог. Требуется ручная визуальная QA на репрезентативных листах.
...
| Дрейф масштаба aux viewport | Трансформация aux-to-main использует исходный масштаб main viewport до global clamp. |
| Удвоенная нормализация размеров | Нормализация выполняется в `DimensionStyleNormalizer` до клонирования. |
| Авто-масштаб на имперских DWG | Синхронизация units/measurement выполняется после `CloseInput(true)`. |
| Остатки оригиналов aux viewport | Добавлено удаление объектов вне main window. |
