...
**Статус:** Диагностика (`[DIM-STYLE]`, viewport-normalize logs, snapshots) пишется в лог. Требуется ручная визуальная QA на репрезентативных листах, включая проверку отсутствия `304.8` в `Dimscale` и `Dimlfac` после merge.
