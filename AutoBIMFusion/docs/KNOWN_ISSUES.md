...
Алгоритм нормализует размеры во время обработки каждого Viewport до клонирования aux-геометрии. Для каждого видимого Model Space размера создаётся или переиспользуется стиль вида `"{Стиль}_{Множитель}"`, очищаются DSTYLE overrides, `Dimscale` запекается в визуальные параметры стиля и приводится к `1.0`; после трансформаций `Dimlfac` приводится к `1.0`.
