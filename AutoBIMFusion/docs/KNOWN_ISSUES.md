...
**Где:** `Application/Merge/BlockInserter.cs`, метод `InsertNativeObjects`, строка ~64

```csharp
sourceDb.WblockCloneObjects(sourceIds, targetMsId, map, DuplicateRecordCloning.Replace, false);
```

**Описание:** Режим `Replace` перезаписывает существующие символьные таблицы (`LayerTableRecord`, `LinetypeTableRecord`, текстовые стили). Если исходный DWG имеет другие параметры слоя «0» — они заменятся в целевом файле.

**Риск:** 
- Неожиданное изменение визуальных свойств уже вставленных объектов.
- При merge множества файлов стили могут переписаться несколько раз.
- Конечный результат зависит от порядка обработки файлов.

**Варианты решения:**

1. **Перейти на `DuplicateRecordCloning.Ignore`:** Новые объекты получат существующие стили целевого документа.

2. **Использовать `DuplicateRecordCloning.MangleName`:** Переименовать все таблицы с префиксом источника.

3. **Явная документация:** Если выбран `Replace` по дизайну, это должно быть задокументировано.
