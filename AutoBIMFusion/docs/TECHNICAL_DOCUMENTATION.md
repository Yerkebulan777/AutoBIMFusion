...
3. Вставляет нативные объекты из временного файла в целевой документ через `BlockInserter.InsertNativeObjects()`:
   - Использует `WblockCloneObjects` с `DuplicateRecordCloning.Replace`.
   - Применяет смещение (offset) в зависимости от ориентации layout'а.

### 2.5 Runtime-поток команд SMART_MERGE_TEXT, MergeTextStyles, CreateETransmitZip

**SMART_MERGE_TEXT:**
- Группирует `DBText` в Model Space по стилю, высоте и геометрической близости (соседству по Y).
- Заменяет цепочки одного стиля на один `MText` для компактности.
- Использует коэффициенты `WordSpacingFactor` (1.5) и `LineHeightFactor` (0.5).
...
| Метод | Сигнатура | Описание |
| :--- | :--- | :--- |
| `InsertNativeObjects` | `public void` | Клонирование объектов из временного файла в целевой DB с WblockCloneObjects |
| `GetInsertPoint` | `private Point3d` | Вычисление точки вставки на основе текущих bounds целевого документа |
| `ApplyOffsetIfNeeded` | `private void` | Применение смещения, если layout ориентирован не как "модель" |
...
2. **WblockCloneObjects мутирует целевой документ** (KI-2):
   - Использование `DuplicateRecordCloning.Replace` перезаписывает слои, типы линий и стили.
   - **Риск:** Неожиданное изменение визуальных свойств уже вставленных объектов.
