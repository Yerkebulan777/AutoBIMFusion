...
public static class Leaders
{
    public static void Draw(object Content, Point3d BasePointArgs, Point3d TextPositionArgs)
    {
        ...
        _ = acMText.AddToDrawingCurrentTransaction();
        using (Leader acLdr = new())
        {
            _ = acLdr.AppendVertex(FlattenBasePoint);
            _ = acLdr.AppendVertex(FlattenTextPosition);
            acLdr.HasArrowHead = true;
            _ = acLdr.AddToDrawingCurrentTransaction();
