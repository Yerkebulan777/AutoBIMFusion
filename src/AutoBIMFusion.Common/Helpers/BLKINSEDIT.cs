...
public static class BLKINSEDIT
{
    public static void MoveBasePoint()
    {
        ...
        var blockRefId = promptResult.Value.GetObjectIds().First();
        using (var tr = db.TransactionManager.StartTransaction())
        {
            ...
            if (!(blockRefId.GetDBObject() is BlockReference blockRefOut)) return;
        ...
        var selectedPoint = Points.GetFromPromptPointResult(pointResult).SCG;
        var FixPosition = selectedPoint - blockRef.Position;
        var BlockReferenceTransformedPoint = selectedPoint.TranformToBlockReferenceTransformation(blockRef);
        ...
        Point3d BlockReferenceTransformedPoint, out Point3d OriginalBlocBasePointInModelSpace)
    {
        ...
        using (var tr = db.TransactionManager.StartTransaction())
        {
            ...
            var FakeBlocBasePointInBlocSpace =
