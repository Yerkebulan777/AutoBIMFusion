...
### KI-6. Малые объекты за рамкой листа

**Серьёзность:** Средняя

**Где:** `OutOfFrameEntityCleaner.FindSmallEntitiesOutsideFrame`

Алгоритм удаляет только объекты с диагональю BoundingBox ≤15 units, если центральная точка bbox по XY находится за рамкой листа. Если рамка листа не найдена, очистка пропускается.

**Решение:** Добавить configurable threshold или визуальную диагностику кандидатов, если реальные проекты потребуют более тонкой настройки.
...
| Тяжёлая диагностика размерных стилей | `DimensionStyleDiagnosticUtils` выполняет снимки в Debug-сборках или при `LOG_LEVEL=DEBUG`. |
...
| Small out-of-frame geometry corruption extents | `OutOfFrameEntityCleaner` (frame-based bbox center detection) |
...
| Смещение крупногабаритных объектов aux VP за рамкой листа | Убран center-based reject для крупных частично пересекающихся объектов; selection снова основан на AABB intersection, как в стабильной ветке `TEST`. |
