...
Архив старых обходов проблемы скрытого масштабирования AutoCAD при `WblockCloneObjects` между базами с разными `Insunits`/`Measurement`. Все фрагменты заменены единым `DatabaseUnitSyncScope` (`src/AutoBIMFusion.Common/AcadSupport/DatabaseUnitSyncScope.cs`).
